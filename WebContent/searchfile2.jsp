...
function search(step,control){
	...
	if(name!=""){
	url = "http://localhost:8080/cmsnew/searchdatabase1?name=" + name + "&step=" + step + "&control=" + control; 
...
function receivedata(step,control){
	if(request.readyState==4){
		if(request.status==200){
			...
			display(responsetext,step,control);
		}
	}
}
...
function display(data,step,control){
	for(i=0;i<(data.length);i++){		
			tempstr = responsetext[i].filename;
			//alert(tempstr);
			if(tempstr.match(".")){
				index = tempstr.indexOf(".");
				tempstr = tempstr.substr(0,index);
			}
			temp = document.createTextNode(responsetext[i].id + ". ");
			...
			str = "window.location='http://localhost:8080/cmsnew/downloadimage?filename=" + responsetext[i].filename + "&path=" + responsetext[i].path + "'";
